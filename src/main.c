...
int main(void)
{
    ...
    SDL_Texture *gpu = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_STREAMING, w, h);
    ...
    unsigned frames = 0;
    float angle = 0.f;
    for (const bool *keys = SDL_GetKeyboardState(NULL); !keys[SDL_SCANCODE_Q]; SDL_PumpEvents())
    {
        frames += 1;
        angle += 0.0001f;
        ...
        float x1 = cosf(angle + HPI) * r + w / 2, y1 = sinf(angle + HPI) * r + h / 2;
        float x2 = cosf(angle + PI) * r + w / 2, y2 = sinf(angle + PI) * r + h / 2;
        float x3 = cosf(angle - HPI) * r + w / 2, y3 = sinf(angle - HPI) * r + h / 2;
        ...
        render_triangle(pixels, w, roundf(x0 * N) * inv_N, roundf(y0 * N) * inv_N, roundf(x1 * N) * inv_N,
                        roundf(y1 * N) * inv_N, roundf(x2 * N) * inv_N, roundf(y2 * N) * inv_N);

        render_triangle(pixels, w, roundf(x0 * N) * inv_N, roundf(y0 * N) * inv_N, roundf(x2 * N) * inv_N,
                        roundf(y2 * N) * inv_N, roundf(x3 * N) * inv_N, roundf(y3 * N) * inv_N);
